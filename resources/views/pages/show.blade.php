    ...
    @if($hasSections)
        <div class="space-y-4 mb-8">
                ...
                <div class="group relative bg-white rounded-lg shadow overflow-hidden">
                    ...
                    <div class="absolute top-2 right-2 z-10 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <form method="POST" action="{{ route('clinic.sites.pages.sections.delete', [$clinic, $site, $page, $sid]) }}" class="inline"
                            ...
                            @csrf @method('DELETE')
                            <button class="bg-red-100 text-red-700 px-2 py-1 rounded text-xs hover:bg-red-200 shadow-sm">削除</button>
                        </form>
                    </div>
            ...
            <div class="mt-2" x-data="{ open: false }">
                <button @click="open = !open" class="w-full border-2 border-dashed border-gray-300 rounded-lg py-3 text-sm text-gray-400 hover:border-indigo-300 hover:text-indigo-500 transition">
                    + セクションを追加
                </button>
                <div x-show="open" style="display:none" class="mt-2 bg-white rounded-lg shadow p-4">
                    <form method="POST" action="{{ route('clinic.sites.pages.sections.add', [$clinic, $site, $page]) }}" class="flex items-end space-x-3">
                        @csrf
                        <div class="flex-1">
                            <label class="block text-xs font-medium text-gray-700 mb-1">見出し</label>
                            <input type="text" name="heading" required class="w-full border border-gray-300 rounded text-sm px-3 py-2" placeholder="例：当院の特徴">
                        </div>
                        <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded text-sm hover:bg-indigo-700">追加</button>
                        <button type="button" @click="open = false" class="bg-gray-200 text-gray-600 px-3 py-2 rounded text-sm">閉じる</button>
                    </form>
                </div>
            </div>
        </div>
